...
Sortable.prototype.onmousemove = function(e) {
  ...
  this.translate(0, touch.clientY - sy)
  if (util.getPosition(touch.clientX, touch.clientY, this.el) !== 0) {
    this.positionHolder(touch)
  }
...
var positionHolder = function (e) {
  ...
  for (var i = children.length - 1; i >= 0; i--) {
    ...
    var prev = util.prev(node, d)
    var next = util.next(node, d)
    var pos = util.getPosition(x, y, node)
    if (pos === 0) continue
    if (tod) {
      if (pos <= 2 && prev !== h) { //prev
        this.holder = util.transit(h, node, 'before')
      } else if (pos > 2 && next !== h) { //prev
        this.holder = util.transit(h, node, 'after')
      }
    }
    else {
      if ((pos%2 === 1) && prev !== h) {
...
Sortable.prototype.translate = function(x, y){
  var s = this.dragEl.style
  x = x || 0
  y = y || 0
  if (has3d) {
